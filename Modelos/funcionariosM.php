...
class funcionariosM extends ConexionBD{
	...
	static public function CrearfuncionarioM($tablaBD,$datosC){
		$pdo = ConexionBD::cBD()->prepare("INSERT INTO $tablaBD(apellido,nombre,cargo,fecha,telefono,
		telefono2,telefono3,correo,correo2,institucion,direccion,foto,creado_por,nota,ciudad,dni,cv,cci,pasaporte) 
		VALUES(:apellido,:nombre,:cargo,:fecha,:telefono,:telefono2,:telefono3,:correo,:correo2,
		:institucion,:direccion,:foto,:creado_por,:nota,:ciudad,:dni,:cv,:cci,:pasaporte)");
//conexion para los valores de nuestras funcionarios Controladores//
		...
		$pdo->bindParam(":cci",$datosC["cci"],PDO::PARAM_STR);
		$pdo->bindParam(":pasaporte",$datosC["pasaporte"],PDO::PARAM_STR);
...
static public function ActualizarfuncionarioM($tablaBD,$datosC){
	...
	$pdo = ConexionBD::cBD()->prepare("UPDATE $tablaBD SET apellido = :apellido ,nombre = :nombre  ,cargo=:cargo, fecha=:fecha, telefono = :telefono,telefono2 = :telefono2,telefono3=:telefono3,correo = :correo,correo2 = :correo2,institucion = :institucion, direccion = :direccion ,foto =:foto,creado_por=:creado_por,nota=:nota,cci=:cci,pasaporte=:pasaporte  WHERE id = :id"); 
...
static public function perfilM($tablaBD,$id){
	$pdo=ConexionBD::cBD()->prepare("SELECT id,apellido,nombre,cargo,fecha,telefono,telefono2,telefono3,correo,correo2,institucion,direccion,foto,nota,ciudad,dni,cci,pasaporte FROM $tablaBD WHERE id=:id");
	$pdo->bindParam("id",$id,PDO::PARAM_INT);
	$pdo->execute();
	return $pdo->fetch();
	$pdo ->close();
